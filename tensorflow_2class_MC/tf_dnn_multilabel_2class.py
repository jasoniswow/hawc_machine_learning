...
parser.add_argument('--featureGroup', type=int, default=9,
                    help='-1=essential, 0=8basic, 9=all40')
...
feature_basic = ['SFCFChi2', 'planeChi2', 'coreFiduScale', 'CxPE40', 'PINC', 'scandelCore', 'fixedFrac', 'disMax']
...
elif (args.featureGroup==0): # basic features
...
# delAngle + delCore separation
print ( "Class weights =================================================================================" )
# The weighted sum of each class for each bin (fHitH is a bin with all high energy stuff)
#           Signal          Background
# fHit0a    154.01147       43331524
# fHit0b    76.700188       18709780
# fHit0c    37.205287       8167552
# fHit1     50.052587       11223499
# fHit2     13.028451       3221108
# fHitH     10.873147       3261717
...
[ wSum_1/154.01147,       wSum_1/43331524],
[ wSum_1/76.700188,       wSum_1/18709780],
[ wSum_1/37.205287,       wSum_1/8167552 ],
[ wSum_1/50.052587,       wSum_1/11223499],
[ wSum_1/13.028451,       wSum_1/3221108 ],
[ wSum_1/10.873147,       wSum_1/3261717 ]]
classWeights_doubleG = [
[ wSum_1/154.01147*2,       wSum_1/43331524],
[ wSum_1/76.700188*2,       wSum_1/18709780],
[ wSum_1/37.205287*2,       wSum_1/8167552 ],
[ wSum_1/50.052587*2,       wSum_1/11223499],
[ wSum_1/13.028451*2,       wSum_1/3221108 ],
[ wSum_1/10.873147*2,       wSum_1/3261717 ]]
classWeights_doubleH = [
[ wSum_1/154.01147,       wSum_1/43331524*2],
[ wSum_1/76.700188,       wSum_1/18709780*2],
[ wSum_1/37.205287,       wSum_1/8167552 *2],
[ wSum_1/50.052587,       wSum_1/11223499*2],
[ wSum_1/13.028451,       wSum_1/3221108 *2],
[ wSum_1/10.873147,       wSum_1/3261717 *2]]
...
def load_data():
    ...
    df_train['pclass1'] = df_train['pclass'].map(lambda x: 1 if (x!=0) else 0)
    df_train_class = df_train[['pclass0','pclass1']]
    ...
    df_test['pclass1'] = df_test['pclass'].map(lambda x: 1 if (x!=0) else 0)
    df_test_class = df_test[['pclass0','pclass1']]
...
def pseudoPrecision(true0, true1, pred, weights):
    ...
    denominator = tf.sqrt(true1P)
    if (denominator!=0):
        res = tf.divide( true0P, denominator )
    else:
        res = -1
    return res
...
pseudoprecisionGG = pseudoPrecision(trueGG, trueBG, predGG, W)
...
with tf.Session() as sess:
    ...
    for epoch in range(1):
        for batch_number in range(total_batch):
            ...
            loss_tr, acc_tr, \
            preGG_tr, preBG_tr, preGG_w_tr, preBG_w_tr, \
            reGG_tr, reBG_tr, reGG_w_tr, reBG_w_tr, \
            pseudoPGG_tr, rejNoneGG_tr, rejNoneGG_tr_, sigRatio_tr = sess.run( \
            [loss_op, accuracy, \
            precision_GG, precision_BG, precision_w_GG, precision_w_BG, \
            recall_GG, recall_BG, recall_w_GG, recall_w_BG, \
            pseudoprecisionGG, rejectionNoneGG, rejectionNoneGG_, significance_ratio ], \
            ...
            loss_te, acc_te, \
            preGG_te, preBG_te, preGG_w_te, preBG_w_te, \
            reGG_te, reBG_te, reGG_w_te, reBG_w_te, \
            pseudoPGG_te, rejNoneGG_te, rejNoneGG_te_, sigRatio_te = sess.run( \
    ...
    for epoch in range(1, training_epochs+1):
        ...
        if (epoch % display_step == 0 or epoch == 1):
            ...
            if (epoch == training_epochs):
                ...
                final_indicators.append("Precision(Bkg_w): %.4f \n"%(preBG_w_tr))
                final_indicators.append("Recall(Sig_w):    %.4f \n"%(reGG_w_tr))
                final_indicators.append("Recall(Bkg_w):    %.4f \n"%(reBG_w_tr))
...
plt.plot(DisplayStep, PrecisionGGPlot_train, linestyle='-', marker='.', markersize=6, color='red', alpha=1.0, label='Training (Signal)')
plt.plot(DisplayStep, PrecisionBGPlot_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Background)')
plt.plot(DisplayStep, PrecisionGGPlot_test, linestyle='-', marker='.', markersize=6, color='red', alpha=0.2, label='Testing (Signal)')
plt.plot(DisplayStep, PrecisionBGPlot_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Background)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, PrecisionGGPlot_w_train, linestyle='-', marker='.', markersize=6, color='red', alpha=1.0, label='Training (Signal)')
plt.plot(DisplayStep, PrecisionBGPlot_w_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Background)')
plt.plot(DisplayStep, PrecisionGGPlot_w_test, linestyle='-', marker='.', markersize=6, color='red', alpha=0.2, label='Testing (Signal)')
plt.plot(DisplayStep, PrecisionBGPlot_w_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Background)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, RecallGGPlot_train, linestyle='-', marker='.', markersize=6, color='red', alpha=1.0, label='Training (Signal)')
plt.plot(DisplayStep, RecallBGPlot_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Background)')
plt.plot(DisplayStep, RecallGGPlot_test, linestyle='-', marker='.', markersize=6, color='red', alpha=0.2, label='Testing (Signal)')
plt.plot(DisplayStep, RecallBGPlot_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Background)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, RecallGGPlot_w_train, linestyle='-', marker='.', markersize=6, color='red', alpha=1.0, label='Training (Signal)')
plt.plot(DisplayStep, RecallBGPlot_w_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Background)')
plt.plot(DisplayStep, RecallGGPlot_w_test, linestyle='-', marker='.', markersize=6, color='red', alpha=0.2, label='Testing (Signal)')
plt.plot(DisplayStep, RecallBGPlot_w_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Background)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.title("(true GoodGamma labeled as GoodGamma) / sqrt(true non-GoodGamma labeled as GoodGamma)", fontsize=20)
matplotlib.rc('font', size=18)
plt.tick_params(labelsize=20)
plt.legend(loc=4)
