...
def load_data():
    # Load files.
    hdf_train = pd.HDFStore('%s/training_sweets_dec20_noise_MPF_allParticle_bin_%d.h5'%(args.fileDir,args.bin),'r')
    hdf_test = pd.HDFStore('%s/testing_sweets_dec20_noise_MPF_allParticle_bin_%d.h5'%(args.fileDir,args.bin),'r')
...
output_text = open('%s/variables_bin_%s_hl_%s'%(out_directory_name,str(args.bin),str(args.hiddenlayer)),'w')
