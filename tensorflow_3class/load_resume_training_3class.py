...
if not os.path.exists(args.modelDir):
    print ( "Please provide model file directory !" )
    sys.exit()
...
# For this problem, we can have 4 to more than 20 input features, with 3 output classes.
...
with tf.Session() as sess:
    ...
    # this only works if all the stores variables have default names
    saver = tf.train.Saver()
    saver.restore(sess, '%s/model.ckpt'%(args.modelDir))

    # another way to load model
    # "meta" file saves graph structure, needed to be imported before restoring checkpoint 
    # load meta graph and checkpoint
    #saver = tf.train.import_meta_graph('%s/model.ckpt.meta'%(args.modelDir))
    #saver.restore(sess, tf.train.latest_checkpoint('%s'%(args.modelDir)))

    #graph = tf.get_default_graph()
    
    # always test all data
    ...
    for epoch in range(1, training_epochs+1):
        ...
        if (epoch % display_step == 0 or epoch == 1):
            ...
            loss_te, acc_te, \
            preGG_te, preBG_te, preH_te, preGG_w_te, preBG_w_te, preH_w_te, \
            reGG_te, reBG_te, reH_te, reGG_w_te, reBG_w_te, reH_w_te, \
...
plt.plot(DisplayStep, PrecisionHPlot_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Hadron)')
...
plt.plot(DisplayStep, PrecisionHPlot_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Hadron)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, PrecisionHPlot_w_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Hadron)')
...
plt.plot(DisplayStep, PrecisionHPlot_w_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Hadron)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, RecallHPlot_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Hadron)')
...
plt.plot(DisplayStep, RecallHPlot_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Hadron)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
...
plt.plot(DisplayStep, RecallHPlot_w_train, linestyle='-', marker='.', markersize=6, color='blue', alpha=1.0, label='Training (Hadron)')
...
plt.plot(DisplayStep, RecallHPlot_w_test, linestyle='-', marker='.', markersize=6, color='blue', alpha=0.2, label='Testing (Hadron)')
plt.ylim([0,1])
plt.xlabel('Epoch Number', fontsize=20)
