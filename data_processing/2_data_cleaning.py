...
hdf_rawMC = pd.HDFStore('data/sweets_dec19_noise_MPF_allParticle_100.h5','r')
...
print ( "Add extra columns indicating classes =====================================================" )
# add an extra column that indicates gamma (as 1) or hadron (as 0)
df['signal'] = df['corsikaParticleId'].map(lambda x: 1 if x==1 else 0).astype(int)
# add an extra column that indicates the good gamma(==0), bad gamma(==1), good hadron(==2), bad hadron(==3)
# to be good event, it requires rec.coreFiduScale<=110 & delAngle<~3degree & delCore<30 
# using np.where, the conditions use bitwise "&" for "and", "|" for "or". If The condition is true, it returns to the first value, and returns the second otherwise
#          goodG   badG   goodH   badH
# signal     1      1       0      0   
# pclass'    0      1       0      1
# pclass     0      1       2      3
df['pclass'] = np.where( (df['coreFiduScale'] <=110) & (df['delAngle'] <=0.06 ) & (df['delCore'] <=30), 0, 1).astype(int)
df.loc[ (df['signal'] ==0) & (df['pclass'] ==0), 'pclass'] = 2
df.loc[ (df['signal'] ==0) & (df['pclass'] ==1), 'pclass'] = 3
print ("Misindentified Gamma: ", df.loc[ (df['signal']==1) & ( (df['pclass']==2) | (df['pclass']==3) ) ].head() )
print ("Misindentified Hadron: ", df.loc[ (df['signal'] ==0) & ( (df['pclass']==0) | (df['pclass']==1) ) ].head() )
'''
df0 = df.loc[df['pclass'] ==0]
df1 = df.loc[df['pclass'] ==1]
df2 = df.loc[df['pclass'] ==2]
df3 = df.loc[df['pclass'] ==3]
print ( df0.head(30)['coreFiduScale'] )
print ( df0.head(30)['delAngle'] )
print ( df0.head(30)['delCore'] )
print ('=============================')
print ( df1.head(30)['coreFiduScale'] )
print ( df1.head(30)['delAngle'] )
print ( df1.head(30)['delCore'] )
print ('=============================')
print ( df2.head(30)['coreFiduScale'] )
print ( df2.head(30)['delAngle'] )
print ( df2.head(30)['delCore'] )
print ('=============================')
print ( df3.head(30)['coreFiduScale'] )
print ( df3.head(30)['delAngle'] )
print ( df3.head(30)['delCore'] )
'''
...
print ("DataFrame with all columns: ", df_bin[nBin].describe())
...
print ( "Normalize features scale to 0-1 ==========================================================" )
df_bin[nBin][features] = df_bin[nBin][features].apply(lambda x: (x - x.min()) / (x.max() - x.min())) # only normalize features, not output class
#df_bin[nBin][features] = df_bin[nBin][features].clip(lower=0.0, upper=1.0) # (optional) set hard cut on features
...
print ( "Print out general information ============================================================" )
...
print ( "Plot feature =============================================================================" )
#plt.clf()
#df_bin[nBin]['TWgt'].hist(bins=100)
#plt.savefig('TWgt.png')
...
hdf_data_train = pd.HDFStore('data/training_sweets_dec19_noise_MPF_allParticle_bin_%d.h5'%(nBin))
hdf_data_train.put('training', data_train[nBin], format='table', data_columns=True)
hdf_data_train.close()
hdf_data_test = pd.HDFStore('data/testing_sweets_dec19_noise_MPF_allParticle_bin_%d.h5'%(nBin))
